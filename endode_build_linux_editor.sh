#!/bin/sh
echo "We now use Docker for the editor build"
exit 0
...
scons platform=linuxbsd target=editor production=yes lto=none d3d12=no deprecated=yes # If deprecated=no then the mono glue (build_assemblies.py specifically) fails
